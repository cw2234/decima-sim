...
class MultiResActorAgent(Agent):
    def __init__(self, sess, node_input_dim, job_input_dim,
                 hid_dims, output_dim, max_depth,
                 executor_levels, mem_caps,
        ...
        # valid mask for node action ([batch_size, total_num_nodes * num_exec_types])
        ...
        # Selected action for node, 0-1 vector
        # ([batch_size, total_num_nodes * num_exec_types])
        self.node_act_vec = tf.placeholder(tf.float32, [None, None])
        # Selected action for job, 0-1 vector
        # ([batch_size, num_jobs, num_limits])
    ...
    def actor_network(self, node_inputs, gcn_outputs, job_inputs,
        ...
        with tf.variable_scope(self.scope):
            ...
            expanded_state = expand_act_on_state(
                merge_node, self.mem_caps)

            node_hid_0 = tl.fully_connected(expanded_state, 32, activation_fn=act_fn)
        ...
        for i in range(len(frontier_nodes)):
            for node in frontier_nodes[i]:
                if job_valid[node.job_dag]:
                    ...
                    node_act = act * len(frontier_nodes) + i
    ...
    def invoke_model(self, obs):
        ...
        node_inputs, job_inputs, \
            job_dags, source_job, num_source_exec, \
            frontier_nodes, exec_commit, moving_executors, \
    ...
    def get_action(self, obs):
        ...
        # parse node action and executor type
