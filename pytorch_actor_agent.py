...
class ActorAgent():
    def __init__(self, sess, node_input_dim, job_input_dim, hid_dims, output_dim,
                 max_depth, executor_levels, eps=1e-6, act_fn=pytorch_tf_op.leaky_relu,
                 optimizer=torch.optim.Adam, lr_rate=0.001):
        ...
        self.gcn = GraphCNN(self.node_input_dim, self.hid_dims, self.output_dim, self.max_depth, self.act_fn)
        self.gsn = GraphSNN(self.node_input_dim + self.output_dim, self.hid_dims, self.output_dim, self.act_fn)

        self.optimizer = optimizer(list(self.gcn.parameters()) + list(self.gsn.parameters()), lr=self.lr_rate)
    ...
    def actor_network(self, node_inputs, gcn_outputs, job_inputs,
        ...
        job_inputs_reshape = job_inputs.view(batch_size, -1, self.job_input_dim)
        ...
        gsn_summ_backward_map_extend = gsn_summ_backward_map.unsqueeze(0).expand(batch_size, -1, -1)
        ...
        gsn_global_summ_extend_job = gsn_global_summ_reshape.repeat(1, gsn_dag_summ_reshape.size(1), 1)
        gsn_global_summ_extend_node = gsn_global_summ_reshape.repeat(1, gsn_dag_summ_extend.size(1), 1)
        ...
        with tf.variable_scope(self.scope):
            ...
            merge_node = torch.cat([
                ...
                gsn_dag_summ_extend,
                gsn_global_summ_extend_node], dim=2)
